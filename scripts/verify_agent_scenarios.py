...
# Pure-completion Pi call: the runner owns the agent loop, so the model call must
# not re-load this project's extension/tools (that would run a second agent loop
# inside every model turn and blow the call timeout). Mirrors the role-spawn flags.
_PI_ISOLATION_FLAGS: tuple[str, ...] = (
...
def _pi_flags(provider: str, model: str) -> list[str]:
    ...
    Mirrors app/thesis/pi_runner.py: an unset flag is absent, never empty.
    """
...
def _pi_completion_argv(provider: str, model: str, prompt: str) -> list[str]:
    ...
    return ["pi", *_pi_flags(provider, model), "--print", "--no-session", *_PI_ISOLATION_FLAGS]
...
def resolve_provider_model(
    ...
    """Explicit flag, then env, then "" — meaning "let Pi use its CLI default"."""
...
def resolve_model_timeout(timeout_arg: str | None, env: object = None) -> int:
    """Per-call model timeout seconds: flag, then STOCKBOT_PI_MODEL_TIMEOUT, then the default."""
    lookup: object = os.environ if env is None else env
    raw = _clean(timeout_arg) or _clean(_lookup_env_value(lookup, "STOCKBOT_PI_MODEL_TIMEOUT"))
    if not raw:
...
def _check_pi_ready(provider: str, model: str, timeout_s: int) -> None:
    try:
        probe = subprocess.run(
            ...
            input="Reply with OK.", capture_output=True, text=True, timeout=timeout_s, check=False,
        )
...
def _pi_model_callable(provider: str, model: str, timeout_s: int) -> Callable[[str], str]:
    ...
    def _call(prompt: str) -> str:
        proc = subprocess.run(
            _pi_completion_argv(provider, model, prompt), input=_clean_prompt(prompt),
...
def _crash_eval_input(scenario: Scenario, wall_ms: float) -> EvalInput:
    return EvalInput(
        ...
        wall_clock_ms=wall_ms, budget_used=0, budget_cap=0,
...
def _build_success_input(
    ...
    tool_call_cap: int = 0, trace: Mapping[str, object] | None = None,
) -> EvalInput:
    ...
    return EvalInput(
        ...
        wall_clock_ms=wall_ms, budget_used=len(tool_names), budget_cap=tool_call_cap,
...
def _live_trace(scenario: Scenario, repo: ResearchRepository, sid: str, jobs: list[Job],
    ...
    return {
        ...
        "committee_freeze_ids": tuple(
            str(run) for run in getattr(sess, "committee_runs", ()) if isinstance(run, str)),
        "roles_completed": _completed_roles(jobs),
    }


def evaluate_and_record(scenario: Scenario, out: dict[str, object], wall_ms: float,
                        tool_call_cap: int = 0) -> EvalInput:
    ...
    return _build_success_input(scenario, answer, tool_names, jobs, sess.status, evidence_ids, wall_ms,
                                tool_call_cap, _live_trace(scenario, repo, sid, jobs, evidence_ids))
...
def _live_kwargs(scenario: Scenario, provider: str, model: str, timeout_s: int) -> _LiveKwargs:
    ...
    return {"question": scenario.question, "objective": scenario.notes or scenario.question,
            ...
            "provider": provider or _DEFAULT_MODEL_LABEL, "model_name": model or _DEFAULT_MODEL_LABEL}

def _invoke_live(kwargs: _LiveKwargs) -> tuple[dict[str, object] | None, float, int]:
    """Live run + the tool-call cap it was given (0 = unlimited), so eval judges real limits only."""
    from app.research.director import DirectorBudgets
    from app.research.repository import ResearchRepository
    from app.research.runner import run_live

    budgets = DirectorBudgets()
    cap = budgets.max_tool_calls if isinstance(budgets.max_tool_calls, int) else 0
    t0 = time.monotonic()
    try:
        out = run_live(**kwargs, repo=ResearchRepository(), budgets=budgets)
    except Exception as exc:  # noqa: BLE001 - the verdict reports the crash, never hides it
        # A crash verdict without its cause is undiagnosable: name it on stderr.
        print(f"CRASH {type(exc).__name__}: {exc}", file=sys.stderr)
        return None, (time.monotonic() - t0) * 1000.0, cap
    return out, (time.monotonic() - t0) * 1000.0, cap

def _run_live_scenario(scenario: Scenario, provider: str, model: str, prompt_version: str,
                       timeout_s: int) -> EvalInput:
    _ = prompt_version  # stamp recorded in the run summary only
    kwargs = _live_kwargs(scenario, provider, model, timeout_s)
    with tempfile.TemporaryDirectory(prefix="agent-scenario-") as tmp:
        old = setup_env(tmp)
        try:
            out, wall_ms, cap = _invoke_live(kwargs)
            if out is None:
                return _crash_eval_input(scenario, wall_ms)
            return evaluate_and_record(scenario, out, wall_ms, cap)
        finally:
            restore_env(old)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--model", default=None,
                        help="Pi model ID used for live execution (or STOCKBOT_PI_MODEL; default: Pi's own CLI default)")
    parser.add_argument("--provider", default=None,
                        help="Pi provider used for live execution (or STOCKBOT_PI_PROVIDER; default: Pi's own CLI default)")
    parser.add_argument("--model-timeout", default=None,
                        help="per-call Pi model timeout seconds (or STOCKBOT_PI_MODEL_TIMEOUT; "
...
def _run_all_scenarios(
    ...
    provider: str, model: str, prompt_version: str, timeout_s: int,
) -> list[ScenarioResult]:
...
def _build_summary(provider: str, model: str, prompt_version: str, results: list[ScenarioResult]) -> dict[str, object]:
    return {
        "provider": provider or _DEFAULT_MODEL_LABEL, "model": model or _DEFAULT_MODEL_LABEL,
...
def _maybe_print_suite_info(
    results: list[ScenarioResult], provider: str, model: str, summary: dict[str, object],
) -> None:
...
def _cli_prereqs(args: argparse.Namespace) -> tuple[str, str, int, list[str], dict[str, Scenario]] | int:
    try:
        provider, model, timeout_s = _prepare_provider_model(args)
    except RuntimeError as exc:
        print(f"SKIP live scenarios: {exc}", file=sys.stderr)
        print("Provide a reachable Pi CLI to evaluate live broad-agent scenarios.", file=sys.stderr)
        return 2
