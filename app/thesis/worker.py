...
def _tick_failed(thesis_id: str, exc: BaseException) -> bool:
    ...
    if isinstance(exc, ValueError) and "closed" in str(exc).lower():
        return True
    log.exception("<worker>: thesis %r tick failed: %s: %s", thesis_id, type(exc).__name__, exc)
    return False


def _report_tick(thesis_id: str, outcome: TickResult, on_tick: Callable[[TickResult], None] | None) -> None:
    ...
    try:
        ...
        log.exception("<worker>: thesis %r on_tick failed: %s", thesis_id, type(exc).__name__)
...
def _run_one_tick(
    *,
    ...
    try:
        outcome = run_monitor_once(repository, thesis_id, source_services, known_at=_loop_known_at(known_at_fn))
