...
def _pi_cmd(prompt: str) -> list[str]:
    ...
    return ["pi", "-p", "--no-session", "--no-builtin-tools", "--no-extensions", "--no-skills", "--no-prompt-templates", "--no-context-files", "--extension", _EXTENSION, *flags, "--", prompt]
...
def _pi_env(data_root: Path | str | None, as_of: str | None, run_id: str | None,
...
def _prepare_launch(prompt: str, data_root: Path | str | None, as_of: str | None,
    ...
    return _PiLaunch(cmd=_pi_cmd(prompt), env=_pi_env(data_root, as_of, run_id, done_p, db_p),
...
def _wait_step(launch: _PiLaunch, proc: subprocess.Popen[bytes], run_id: str | None,
...
def _verdict(launch: _PiLaunch, proc: subprocess.Popen[bytes], thesis_id: str, trigger_id: str,
