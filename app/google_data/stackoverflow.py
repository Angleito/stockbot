...
def _submit_error(result: object) -> dict[str, object] | None:
    ...
    if isinstance(result, dict) and "status" in result:
        return result
    return _executor_error(result if isinstance(result, dict)
...
def _last_covered(result: dict[str, object],
                  activity: list[dict[str, object]]) -> object:
...
def _stale_warning(end_date: str, last_covered: object) -> list[str]:
    ...
    if lag > _STALE_LAG_DAYS:
        return [f"stale_snapshot: last covered {last_covered} lags request "
                f"end {end_date} by {lag}d; treat as historical activity, not current adoption"]
    return []


def _ok_payload(activity: list[dict[str, object]], last_covered: object,
...
def get_tag_activity(tags: list[str] | str | None, *, start_date: str | None = None,
    ...
    result = _submit(_TEMPLATE, {"tags": cleaned, "start_date": start_date,
