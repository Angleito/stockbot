...
def _bq_limits_from_env() -> tuple[int, int, int]:
    return (_bq_env_int("BIGQUERY_MAX_BYTES_PER_QUERY", 1073741824),
...
def _bq_limits_from_config() -> tuple[int, int, int] | dict[str, object] | None:
    ...
    try:
        ...
        return None
    try:
        return (_cfg2.get_bq_max_bytes_per_query(),
...
def _cand_matches_scope_identity(cand: dict[str, object], scope_table: str,
                                 scope_kind: str, term: str) -> bool:
    return (str(cand.get("table") or "") == scope_table
            and str(cand.get("list_kind") or "") == scope_kind
            and str(cand.get("term") or "") == (term or ""))


def _cand_period_in_scope(cand: dict[str, object], week_start: str,
...
def _scope_pair_for(cand: object, scope_table: str, scope_kind: str, term: str,
                    week_start: str, week_end: str, geos: list[str],
    ...
    if not _cand_matches_scope_geo(cand, geos, basis):
        return None
    return [str(cand.get("observation_id") or ""),
            str(cand.get("content_hash") or "")]


def _collect_scope_pairs(candidates: list[dict[str, object]], scope_table: str,
                         scope_kind: str, term: str, week_start: str,
                         week_end: str, geos: list[str], basis: str) -> list[list[str]]:
    pairs: list[list[str]] = []
    for cand in candidates:
        pair = _scope_pair_for(cand, scope_table, scope_kind, term,
                               week_start, week_end, geos, basis)
        if pair is None:
            continue
        pairs.append(pair)
    return pairs
...
def _cand_matches_target(cand: object, target_table: str, target_kind: str,
                         target_geo: str, target_basis: str) -> bool:
    if not isinstance(cand, dict):
        return False
    return (str(cand.get("table") or "") == target_table
...
def _collect_target_periods(candidates: list[dict[str, object]], target_table: str,
                            target_kind: str, target_geo: str, target_basis: str,
                            week_start: str, week_end: str,
                            geos: list[str]) -> set[str]:
    periods: set[str] = set()
    if not _target_gate_open(target_geo, geos):
        return periods
    for cand in candidates:
        if not _cand_matches_target(cand, target_table, target_kind,
                                    target_geo, target_basis):
            continue
...
def expected_inputs_hash(scope: dict[str, object], term: str, table: str, list_kind: str,
    ...
    pairs = _collect_scope_pairs(candidates, scope_table, scope_kind, term,
                                 week_start, week_end, geos, basis or "")
    target_table = table or ""
    target_kind = list_kind or ""
    target_geo = geo or ""
    target_basis = basis or ""
    periods_covered = sorted(_collect_target_periods(
        candidates, target_table, target_kind, target_geo, target_basis,
        week_start, week_end, geos))
...
def _completion_row_eligible(row: dict[str, object]) -> bool:
    return (row.get("pipeline") == _CHECKPOINT_PIPELINE
...
def _warehouse_key(row: dict[str, object], table: str, metrics_canon: str,
                   evidence_canon: str) -> tuple[str, str, str, str, str, str, str, str]:
    return (str(row.get("table") or table), str(row.get("period") or ""),
...
def _warehouse_staged(row: dict[str, object], table: str, metrics: dict[str, object],
                      evidence: list[object]) -> dict[str, object]:
    return {
...
def _warehouse_entry(row: object, table: str,
                     prefix: str) -> tuple[tuple[str, str, str, str, str, str, str, str], dict[str, object]] | None:
    ...
    return (_warehouse_key(row, table, metrics_canon, evidence_canon),
            _warehouse_staged(row, table, metrics, evidence))
...
def _backfill_feature_payload(row: dict[str, object],
                              decoded: dict[str, object]) -> dict[str, object]:
    return {
...
def _finish_backfill(proot: Path, marker: Path,
                     feature_rows: list[dict[str, object]]) -> int:
    try:
...
def _cached_week_in_scope(cached: dict[str, object], params: dict[str, object]) -> bool:
    ...
    return not (week < str(params.get("week_start") or "")
                or week > str(params.get("week_end") or ""))
...
def _cache_in_scope(cached: dict[str, object], params: dict[str, object]) -> bool:
    ...
    for decider in (_national_scope_decision, _dma_scope_decision,
                    _country_scope_decision):
...
def _build_stored_row(obs: dict[str, object], metrics: dict[str, object],
                      evidence: list[object], observation_id: str, content_hash: str,
                      known_at: str, retrieved_at: str) -> dict[str, object]:
    return {
...
def _store_one_observation(obs: dict[str, object], first_known: dict[tuple[str, str], str],
                           retrieved_at: str,
                           expected: dict[tuple[object, str], set[tuple[str, str]]]) -> dict[str, object]:
    ...
    return _build_stored_row(obs, metrics, evidence, observation_id, content_hash,
                             known_at, retrieved_at)


def _store_observations(data_root: Path | str | None, observations: list[dict[str, object]],
...
def _archive_raw(data_root: Path | str | None, template: str, job_id: str, table: str,
...
def _decode_archive_payload(metadata: object, raw: bytes,
                            params: dict[str, object]) -> list[object] | None:
    try:
...
def _archive_record_payload(record: ArchiveRecord, params: dict[str, object]) -> list[object] | None:
    try:
        return _decode_archive_payload(record.metadata,
...
def _archived_rows(data_root: Path | str | None, template: str, job_id: str,
                   params: dict[str, object]) -> list[object] | None:
    ...
    try:
        ...
        return None
    try:
        return _best_archive_payload(
            _raw_archive_r.iter_archive("google", template, job_id, root=root), params)
    except Exception:  # noqa: BLE001 - intentional best-effort boundary, never aborts
        return None


def _mark_complete(data_root: Path | str | None, checkpoint_key: str, refresh: str,
...
def _enumerate_refreshes(table: str, start_date: str, end_date: str, executor: _Executor | None,
                         data_root: Path | str | None) -> list[str] | None:
...
def _week_params_error(week_start: str | None,
                       week_end: str | None) -> dict[str, object] | None:
...
def _collect_params_error(interval: str | None, start_date: str | None,
                          end_date: str | None, week_start: str | None,
                          week_end: str | None,
                          geos: list[str]) -> dict[str, object] | None:
    checks = (_interval_error(interval), _dates_error(start_date, end_date),
              _week_params_error(week_start, week_end), _geos_error(geos))
    for check in checks:
...
def _default_collect_weeks(end_date: str, week_start: str | None,
                           week_end: str | None) -> tuple[str, str]:
...
def _refresh_query_params(group: dict[str, object], refresh: str, week_start: str,
                          week_end: str) -> dict[str, object]:
    base: dict[str, object] = {"start_date": refresh, "end_date": refresh,
...
def _completed_for(data_root: Path | str | None,
                   groups: list[dict[str, object]]) -> set[str]:
    if data_root is None:
        return set()
...
def _submit_or_error(template: str, params: dict[str, object], executor: _Executor | None,
...
def _resolve_rows(template: str, params: dict[str, object],
                  data_root: Path | str | None,
                  result: dict[str, object]) -> tuple[dict[str, object] | None, list[object]]:
    ...
    if cached and not rows:
        ...
        if recovered is None:
            return _wrap_error({"error": f"cached Trends result unavailable for {template}; refusing to checkpoint",
                                "error_type": "source_unavailable", "source": "bigquery"}), []
        return None, list(recovered)
    return None, rows


def _maybe_archive(data_root: Path | str | None, template: str, job_id: str, table: str,
                   params: dict[str, object], rows: list[object], cached: bool) -> None:
...
def _cached_row_refresh(cached: dict[str, object], refresh: str) -> str:
    return (_cached_metrics_refresh(cached) or _cached_record_refresh(cached)
            or str(cached.get("week") or cached.get("period") or refresh))


def _merge_cached_rows(cached_rows: list[dict[str, object]], refresh: str,
                       merged: dict[tuple[object, object, object, object, object, object], dict[str, object]]) -> None:
    for cached in cached_rows:
        key = (cached["table"], _cached_row_refresh(cached, refresh),
...
def _row_markers(staged: dict[str, object], template: str, table: str, refresh: str) -> None:
    ...
    staged["_week"] = str(staged.get("week") or staged.get("period")
                          or staged.get("source_period") or refresh)
    staged["_kind"] = (staged.get("list_kind") or staged.get("list")
                       or _TEMPLATE_LIST_KIND.get(template, "top"))


def _tag_row(row: object, template: str, table: str, refresh: str,
             group: dict[str, object], job_id: object) -> dict[str, object] | None:
    if not isinstance(row, dict):
        return None
    staged = dict(row)
...
def _as_staged(row: dict[str, object]) -> dict[str, object]:
    ...
    if "_table" not in staged_row:
        ...
        staged_row["_refresh"] = str(as_dict(staged_row.get("metrics"), what="metrics").get("refresh_date")
                                     or id_refresh or staged_row.get("period"))
...
def _table_sets() -> tuple[set[str], set[str]]:
    return ({_template_table("trends_us_top"), _template_table("trends_us_rising"),
...
def _dma_scope_set(groups: list[dict[str, object]]) -> set[str]:
    return {d for g in groups if g.get("kind") == "us" and not g.get("national")
            for d in as_str_list(g.get("dmas"), what="dmas")}


def _route_staged(staged_row: dict[str, object], row: dict[str, object], table: object,
                  us_tables: set[str], intl_tables: set[str], national: bool,
                  dma_set: set[str],
                  national_rows: list[dict[str, object]], dma_rows: list[dict[str, object]],
                  intl_rows: list[dict[str, object]]) -> None:
...
def _split_staged(merged: dict[tuple[object, object, object, object, object, object], dict[str, object]],
                  groups: list[dict[str, object]]) -> tuple[list[dict[str, object]], list[dict[str, object]], list[dict[str, object]], set[str]]:
    ...
    for row in merged.values():
        ...
        _route_staged(staged_row, row, table, us_tables, intl_tables, national,
                      dma_set, national_rows, dma_rows, intl_rows)
...
def _series_point(row: dict[str, object]) -> dict[str, object]:
    return {"table": row.get("_table"), "period": row.get("_week"),
            "geo": row.get("_geo"), "term": row.get("term"),
            "list_kind": row.get("_kind"), "rank": _series_metric(row, "rank"),
            "score": _series_metric(row, "score")}


def _group_series(winners: dict[tuple[object, object, object, object, object], dict[str, object]]) -> tuple[dict[tuple[object, object, object, object, str], list[dict[str, object]]], dict[tuple[object, object, object, str], set[str]]]:
...
def _pick_winners(rows: list[dict[str, object]]) -> dict[tuple[object, object, object, object, object], dict[str, object]]:
    winners: dict[tuple[object, object, object, object, object], dict[str, object]] = {}
    for row in rows:
        wkey = (row.get("_table"), row.get("_kind"), row.get("_geo"),
                row.get("term"), row.get("_week"))
...
def _series_feature_map(signals_mod: ModuleType, by_series: dict[tuple[object, object, object, object, str], list[dict[str, object]]], periods_by_series: dict[tuple[object, object, object, str], set[str]]) -> dict[tuple[object, object, object, object, str], dict[str, object]]:
...
def _collect_dma_groups(winners: dict[tuple[object, object, object, object, object], dict[str, object]],
                        dma_set: set[str]) -> dict[tuple[object, object, object, str], list[dict[str, object]]]:
...
def _dma_key_matches(skey: tuple[object, object, object, object, str],
                     gkey: tuple[object, object, object, str], dma_set: set[str]) -> bool:
    return (skey[0] == gkey[0] and skey[1] == gkey[1] and skey[2] == gkey[2]
            and skey[4] == gkey[3] and str(skey[3]) in dma_set)


def _spread_dma_diff(signals_mod: ModuleType, gkey: tuple[object, object, object, str],
                     grows: list[dict[str, object]], dma_set: set[str],
                     series_features: dict[tuple[object, object, object, object, str], dict[str, object]]) -> None:
...
def _apply_dma_diffusion(signals_mod: ModuleType, winners: dict[tuple[object, object, object, object, object], dict[str, object]], dma_set: set[str], series_features: dict[tuple[object, object, object, object, str], dict[str, object]]) -> None:
    if not dma_set:
        return
...
def _features_for(signals_mod: ModuleType, winners: dict[tuple[object, object, object, object, object], dict[str, object]], dma_set: set[str]) -> dict[tuple[object, object, object, object, str], dict[str, object]]:
...
def _normalize_staged(rows: list[dict[str, object]],
                      series_features: dict[tuple[object, object, object, object, str], dict[str, object]],
                      retrieved_at: str,
                      data_root: Path | str | None) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    observations: list[dict[str, object]] = []
    effective: list[dict[str, object]] = []
    winner_ids = {id(w) for w in _pick_winners(rows).values()}
    for row in rows:
        basis = _series_basis(row)
        features = series_features.get(
            (row.get("term"), row.get("_table"), row.get("_kind"), row.get("_geo"), basis))
        metrics_raw: object = row.get("metrics")
        metrics: dict[str, object] = metrics_raw if isinstance(metrics_raw, dict) else {}
        aggregate = (row.get("dma_count") is not None or row.get("region_count") is not None
                     or metrics.get("dma_count") is not None or metrics.get("region_count") is not None)
        norm = _normalize_row(row, retrieved_at, data_root,
                              features=_strip_diffusion(features) if aggregate else features)
...
def _feature_infos(observations: list[dict[str, object]], dma_set: set[str],
                   week_start: str, week_end: str) -> list[tuple[dict[str, object], str, str, dict[str, object], str, str, dict[str, object]]]:
    ...
    for obs in observations:
        ...
        metrics = dict(as_dict(obs.get("metrics"), what="metrics"))
        evidence = list(as_list(obs.get("evidence"), what="evidence"))
        oid, ch = _observation_identity(obs["table"], obs["period"], obs["geo"],
                                        obs["term"], obs["list_kind"], metrics, evidence)
        ...
        if geo == "US":
            ...
            geos = [geo.split(":")[0]]
        scope = _feature_scope_json(week_start=week_start, week_end=week_end, geos=geos,
...
def _feature_candidates(infos: list[_FeatureInfo], effective: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    return [{"observation_id": oid, "content_hash": ch,
             "table": str(obs.get("table") or ""), "period": str(obs.get("period") or ""),
             "geo": str(obs.get("geo") or ""), "term": str(obs.get("term") or ""),
             "list_kind": str(obs.get("list_kind") or ""),
             "metrics": as_dict(obs.get("metrics"), what="metrics")}
            for obs, oid, ch, _, _, _, _ in infos if id(obs) in effective_ids]


def _feature_calc_key(row: dict[str, object]) -> tuple[str, str, str, str]:
    return (str(row.get("observation_id") or ""), str(row.get("feature_scope_hash") or ""),
            str(row.get("calc_version") or ""), str(row.get("inputs_hash") or ""))


def _track_calc(first_calc: dict[tuple[str, str, str, str], str],
...
def _feature_rows(infos: list[_FeatureInfo], candidates: list[dict[str, object]], calc_version: str, retrieved_at: str,
                  first_calc: dict[tuple[str, str, str, str], str]) -> list[dict[str, object]]:
    empty_hash = expected_inputs_hash({}, "", "", "", "", "", [])
    rows: list[dict[str, object]] = []
    for obs, oid, ch, scope, shash, basis, feats in infos:
        expected = expected_inputs_hash(scope, str(obs.get("term") or ""),
                                        str(obs.get("table") or ""),
                                        str(obs.get("list_kind") or ""), basis,
                                        str(obs.get("geo") or ""), candidates)
        if expected == empty_hash:
            continue
        calc_at = first_calc.get((oid, shash, calc_version, expected), retrieved_at)
        rows.append({"observation_id": oid, "feature_scope_hash": shash,
                     "feature_scope_json": json.dumps(scope, sort_keys=True),
                     "features_json": json.dumps(feats, sort_keys=True, default=str),
                     "calc_version": calc_version, "calculated_at": min(calc_at, retrieved_at),
                     "inputs_hash": expected})
    return rows


def _write_feature_rows(parquet_mod: ModuleType, proot: Path,
                        observations: list[dict[str, object]], effective: list[dict[str, object]],
                        dma_set: set[str], week_start: str, week_end: str,
                        calc_version: str, retrieved_at: str) -> None:
    ...
    existing: list[dict[str, object]] = [r for r in raw_existing if isinstance(r, dict)] if isinstance(raw_existing, list) else []
    parquet_mod.write_rows(
        "google_signal_features",
        _feature_rows(infos, _feature_candidates(infos, effective), calc_version,
                      retrieved_at, _feature_calc_map(existing)), root=proot)


def _warehouse_identities(data_root: Path | str | None, table: str,
                            refresh: str) -> set[tuple[str, str]]:
    ...
    for cached in _warehouse_rows(data_root, table, refresh):
        ...
        actual.add(_observation_identity(
            ...
            str(cached.get("geo") or ""), str(cached.get("term") or ""),
            str(cached.get("list_kind") or ""), metrics, evidence))
    return actual


def _verify_partition(data_root: Path | str | None, table: str, refresh: str,
                      expected: dict[tuple[object, str], set[tuple[str, str]]]) -> None:
...
def _verify_warehouse(data_root: Path | str | None,
...
def _mark_fetched(data_root: Path | str | None,
                  fetched: list[tuple[str, str, str, str, int, list[str]]]) -> None:
    for _key, _template, _refresh, _hash, _count, _tables in fetched:
        _mark_complete(data_root, _key, _refresh, _hash, _count)


def _persist_collect(data_root: Path | str | None, observations: list[dict[str, object]],
                     effective: list[dict[str, object]],
                     fetched: list[tuple[str, str, str, str, int, list[str]]],
                     dma_set: set[str], week_start: str, week_end: str,
                     retrieved_at: str, signals_mod: ModuleType) -> None:
    ...
    if proot is not None and parquet_mod is not None:
        ...
        _write_feature_rows(parquet_mod, proot, observations, effective, dma_set,
                            week_start, week_end, calc_version, retrieved_at)
    _verify_warehouse(data_root, fetched, expected)
    _mark_fetched(data_root, fetched)


def _collect_response(observations: list[dict[str, object]], limit: int,
                      used_templates: list[str], refresh_seen: set[str],
                      term: str | None) -> dict[str, object]:
    ...
    return {"status": "ok", "source": SOURCE, "observations": returned,
            "rows": returned, "count": len(returned),
            "coverage": {"periods_covered": weeks, "geos_covered": geos_covered,
                         "templates": used_templates, "refresh_dates": sorted(refresh_seen)},
            "continuation": continuation, "warnings": ["truncated"] if continuation else []}


def _fetch_all(groups: list[dict[str, object]], week_start: str, week_end: str,
               start_date: str, end_date: str, executor: _Executor | None, data_root: Path | str | None,
               completed: set[str],
               merged: dict[tuple[object, object, object, object, object, object], dict[str, object]],
               used_templates: list[str],
               fetched: list[tuple[str, str, str, str, int, list[str]]],
               refresh_seen: set[str]) -> dict[str, object] | None:
    for group in groups:
        for template in _templates_for(group):
            ...
            if refreshes is None:
                return _wrap_error({"error": "refresh enumeration failed",
                                    "error_type": "source_unavailable", "source": "bigquery"})
            for refresh in (refreshes or [end_date]):
                ...
                refresh_error = _collect_one_refresh(
                    template, table, refresh, params, group, checkpoint_key,
                    completed, merged, used_templates, fetched, refresh_seen,
                    executor, data_root)
...
def _collect_end(merged: dict[tuple[object, object, object, object, object, object], dict[str, object]],
                 groups: list[dict[str, object]], signals_mod: ModuleType,
                 used_templates: list[str],
                 fetched: list[tuple[str, str, str, str, int, list[str]]],
                 refresh_seen: set[str], data_root: Path | str | None,
                 week_start: str, week_end: str, limit: int,
                 term: str | None) -> dict[str, object]:
    try:
    ...
    if data_root is not None and (observations or fetched):
        try:
            _persist_collect(data_root, observations, effective, fetched, dma_set,
                             week_start, week_end, retrieved_at, signals_mod)
...
def _merge_tagged(rows: list[object], template: str, table: str, refresh: str,
                  group: dict[str, object], job_id: object,
                  merged: dict[tuple[object, object, object, object, object, object], dict[str, object]]) -> set[str]:
    tables_seen: set[str] = set()
    for row in rows:
        staged = _tag_row(row, template, table, refresh, group, job_id)
        if staged is None:
            continue
        key = (staged["_table"], staged["_refresh"], staged["_week"], str(staged["_geo"]),
               str(staged.get("term")), staged["_kind"])
        merged.setdefault(key, staged)
        tables_seen.add(str(staged["_table"]))
    return tables_seen

def _cached_rows_for(data_root: Path | str | None, table: str, refresh: str,
                     params: dict[str, object]) -> list[dict[str, object]]:
    rows = [c for c in _warehouse_rows(data_root, table, refresh)
...
def _replay_completed(template: str, refresh: str, params: dict[str, object],
                      table: str, checkpoint_key: str, completed: set[str],
                      merged: dict[tuple[object, object, object, object, object, object], dict[str, object]],
                      used_templates: list[str],
                      data_root: Path | str | None) -> tuple[bool, dict[str, object] | None]:
...
def _merge_fresh(template: str, table: str, refresh: str, params: dict[str, object],
                 group: dict[str, object], checkpoint_key: str,
                 merged: dict[tuple[object, object, object, object, object, object], dict[str, object]],
                 used_templates: list[str],
                 fetched: list[tuple[str, str, str, str, int, list[str]]],
                 data_root: Path | str | None,
    ...
    if resolve_err is not None:
        return resolve_err
    job_id = result.get("job_id")
    _maybe_archive(data_root, template, str(job_id or template), table,
                   params, rows, cached)
    ...
    if data_root is not None:
        fetched.append((checkpoint_key, template, refresh,
                        _payload_hash(rows, data_root), len(rows),
                        sorted(tables_seen)))
    return None


def _collect_one_refresh(
        template: str, table: str, refresh: str, params: dict[str, object],
        group: dict[str, object], checkpoint_key: str, completed: set[str],
        merged: dict[tuple[object, object, object, object, object, object], dict[str, object]],
        used_templates: list[str], fetched: list[tuple[str, str, str, str, int, list[str]]],
        refresh_seen: set[str], executor: _Executor | None,
        data_root: Path | str | None) -> dict[str, object] | None:
    refresh_seen.add(refresh)
    replayed, replay_error = _replay_completed(
        template, refresh, params, table, checkpoint_key,
        completed, merged, used_templates, data_root)
    ...
    if err is not None:
        return err
    return _merge_fresh(template, table, refresh, params, group, checkpoint_key,
                        merged, used_templates, fetched, data_root, result)


def collect_trends(*, start_date: str | None, end_date: str | None, geos: list[str] | str,
                   limit: int = 100, data_root: Path | str | None = None,
                   executor: _Executor | None = None, week_start: str | None = None,
                   week_end: str | None = None, interval: str | None = "daily",
                   term: str | None = None) -> dict[str, object]:
    """Collect top/rising lists; each row becomes an idempotent candidate."""
    geos = _normalize_geos(geos)
    ready = _readiness_error()
    if ready:
        return ready
    params_error = _collect_params_error(interval, start_date, end_date,
                                         week_start, week_end, geos)
    if params_error:
        return params_error
    assert isinstance(start_date, str) and isinstance(end_date, str)
    ...
    fetched: list[tuple[str, str, str, str, int, list[str]]] = []
    fetch_error = _fetch_all(groups, week_start, week_end, start_date, end_date,
                             executor, data_root, completed, merged,
                             used_templates, fetched, refresh_seen)
    if fetch_error is not None:
        return fetch_error
    return _collect_end(merged, groups, _signals, used_templates, fetched,
                        refresh_seen, data_root, week_start, week_end, limit, term)
...
def _cached_roundtrip(row: dict[str, object]) -> tuple[dict[str, object], list[object]] | None:
    if row.get("_cached") and isinstance(row.get("metrics"), dict):
        ...
        return (dict(as_dict(row.get("metrics"), what="metrics")),
                list(as_list(row.get("evidence"), what="evidence")))
    return None
...
def _fresh_metrics(row: dict[str, object], refresh: str, week: str) -> dict[str, object]:
    metrics: dict[str, object] = {"rank": row.get("rank"), "score": row.get("score"),
...
def _fresh_evidence(row: dict[str, object], table: object) -> list[object]:
    return [{"table": table, "row": {k: v for k, v in row.items() if not k.startswith("_")},
             "job_id": row.get("job_id"), "template": row.get("_template")}]


def _normalize_row(row: dict[str, object], retrieved_at: str, data_root: Path | str | None,
