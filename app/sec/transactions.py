...
_EXCHANGE = re.compile(r"[\d.]+\s*(?:shares?|for each|per)\s+[^.]{0,40}?shares?", re.IGNORECASE)
...
_ACQUIRE_RE = re.compile(r"([A-Z][A-Za-z0-9&.,'’\- ]{1,80}?)\s+agreed\s+to\s+acquire", re.IGNORECASE)
_MERGER_RE = re.compile(r"merger\s+(?:with|of)\s+([A-Z][A-Za-z0-9&.,'’\- ]{1,80})", re.IGNORECASE)
...
_STATUS_PATTERNS = (
    ("accepted", re.compile(r"accept\w+\s+for\s+payment|acceptance\s+of\s+the\s+offer", re.IGNORECASE)),
    ("withdrawn", re.compile(r"withdraw\w+(\s+of)?\s+the\s+offer|offer\s+(\w+\s+){0,3}withdrawn", re.IGNORECASE)),
    ("terminated", re.compile(r"terminat\w+(\s+of)?\s+the\s+(offer|merger|transaction|agreement)", re.IGNORECASE)),
    (
        "completed",
        re.compile(
...
_STATUS_VOCABULARY = frozenset({"unknown", "accepted", "completed", "withdrawn", "terminated"})
...
def extract_transaction_parties(obj: object | None = None, *, text: str | None = None) -> dict[str, object]:
    ...
    """
    try:
        parties, structured = _structured_parties_of(obj) if obj is not None else (_empty_parties(), False)
...
def _money_fee_gap(m: re.Match[str], fee_spans: list[tuple[int, int]]) -> int:
    gaps = [
        0 if m.start() < e and s < m.end() else (s - m.end() if m.end() <= s else m.start() - e) for s, e in fee_spans
    ]
    return min(gaps)
...
def _fallback_target_of(parties: dict[str, object]) -> str | None:
    return _str_or_none(parties.get("subject_name")) or _str_or_none(parties.get("target_name"))


def _resolve_deal_target(target: str, subject_name: str | None, parties: dict[str, object]) -> str:
    return _str_or_none(target) or _str_or_none(subject_name) or _fallback_target_of(parties) or ""
...
def _extraction_method_of(
    parties: dict[str, object],
    obj: object | None,
    *,
    filer_cik: str | int | None,
    filer_name: str | None,
    ...
    if (
        filer_cik is not None
        or filer_name is not None
        or subject_cik is not None
        or subject_name is not None
        or acquirer is not None
        or buyer is not None
    ):
        method = "structured-header" if obj is not None else method
    return method


def _resolve_acquirer_name(acquirer: object, buyer: object, parties: dict[str, object]) -> str | None:
    # Explicit args win, structured evidence next, spans last; the filer is
    # never copied into subject/target/acquirer.
    return _str_or_none(acquirer) or _str_or_none(buyer) or _str_or_none(parties.get("acquirer_name"))
...
def _deal_text_spans(text: str | None) -> tuple[str | None, str | None]:
    ...
    return (exchange.group(0).strip() if exchange else None, expiry.group(0).strip() if expiry else None)
...
def _merged_field_value(previous: Transaction, current: Transaction, name: str) -> object:
    ...
    if name == "source_accessions":
        return tuple(previous.source_accessions) + tuple(
            a for a in current.source_accessions if a not in previous.source_accessions
        )
...
def update_transaction(previous: Transaction, current: Transaction) -> Transaction:
...
def get_transaction_status(
    ticker_or_cik: str | int, *, as_of: str | None = None, limit: int | None = 10
) -> list[Transaction]:
    filings = list_sec_filings(ticker_or_cik, forms=list(TRANSACTION_FORMS), as_of=as_of, limit=limit)
