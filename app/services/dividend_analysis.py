...
def _cadence_context(
    paid_events: Sequence[Mapping[str, object]] | None,
) -> tuple[dict[str, object], list[tuple[_dt.date, float]], float, bool]:
...
def _latest_change(
    ...
    if len(series) < 2:
        return None, None
    (_prior_date, prior), (new_date, new) = series[-2], series[-1]
    if new > prior:
        return ({"pct": _change_pct(prior, new), "amount": round(new, 4), "date": new_date.isoformat()}, None)
    if new < prior:
        return (
            None,
            {
                "pct": _change_pct(prior, new),
                "prior": round(prior, 4),
                "new": round(new, 4),
...
def _freeze_run(
    series: Sequence[tuple[_dt.date, float]], cadence: Mapping[str, object], observed: bool
...
def _possible_suspension(
    observed: bool, series: Sequence[tuple[_dt.date, float]], gap_days: float, as_of: _dt.date | str | None
) -> bool:
    """True when the last payment is >3 median gaps before as_of."""
    as_of_day = _parse_day(as_of) if as_of is not None else None
    return bool(observed and series and as_of_day is not None and (as_of_day - series[-1][0]).days > 3 * gap_days)


def _reinstatement(
    observed: bool, series: Sequence[tuple[_dt.date, float]], gap_days: float
) -> dict[str, object] | None:
    """First post-gap resumption date once any inter-payment gap exceeds 3x median."""
    if not observed or len(series) < 2:
        return None
    for (prev_day, _), (day, _) in itertools.pairwise(series):
...
def lifecycle_from_events(
    paid_events: Sequence[Mapping[str, object]] | None, *, as_of: _dt.date | str | None = None
) -> dict[str, object]:
    ...
    result.update(
        {
            ...
            "regular_basis_growth": (
                _dividend_growth(annual_regular) if _has_consecutive_run(list(annual_regular), 5) else None
            ),
        }
    )
    return result
