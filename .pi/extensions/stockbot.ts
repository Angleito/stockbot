...
export default async function stockbotExtension(pi: ExtensionAPI) {
 ...
 // Fresh bridge per `bun run stockbot`: kill child on quit/reload/new
 // so _sessions never survive into the next process. Orphans from a
 // crash are pre-killed by the bun script.
