...
from app.sec import offerings


def _filing(form: str, filed_at: str, accession: str, company: str = "ACME") -> SimpleNamespace:
    return SimpleNamespace(form=form, filed_at=filed_at, accession_no=accession, company=company)
...
def test_history_links_and_statuses(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert [o.filed_at for o in out] == ["2024-01-10", "2024-02-01", "2024-02-05", "2024-03-01"]
...
def test_terms_forms_skips_unconsumed_fetches(monkeypatch: pytest.MonkeyPatch) -> None:
    filings = [_filing("S-3", "2024-01-10", "s3"), _filing("424B5", "2024-02-01", "b5")]
    fetched: list[str] = []
...
def test_atm_detected_from_type_text() -> None:
    rec = offerings.normalize_offering(
        "a", "424B5", issuer="ACME", filed_at="2024-02-01", terms={"offering_type": "At The Market offering"}
    )
