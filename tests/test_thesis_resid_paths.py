...
def _section(key: str, rows: list[object], tid: str) -> dict[str, JSONValue]:
    ...
    rows_json: list[JSONValue] = [
        r if isinstance(r, (str, int, float, bool)) or r is None or isinstance(r, (list, dict)) else str(r)
        for r in rows
    ]
    out[key] = rows_json
    return out
...
def test_snapshot_ids_filters_to_string_ids() -> None:
    rows = [{"claim_id": "c1"}, {"claim_id": 5}, {"other": "x"}, "junk", None, {"claim_id": "c2"}]
...
def _state(thesis: dict[str, JSONValue]) -> ThesisStateSnapshot:
    return ThesisStateSnapshot(
        thesis_id="t1", version=1, effective_at=JUN, recorded_at=JUN, reason="test", thesis=thesis
    )
...
def test_snapshot_coverage_folds_eligible_rules_only() -> None:
    snap = _snap(
        {"scope": "NVDA", "claims": [{"claim_id": "c1"}], "expressions": [{"expression_id": "e1"}]},
        {
            "rules": [
                {
...
def test_backdated_section_folds_empty_is_clean() -> None:
    r = _repo(Path("/tmp"))
    q, m, w = (_section("questions", [], "t1"), _section("memories", [], "t1"), _section("rules", [], "t1"))
...
def test_backdated_section_folds_all_arms(tmp_path: Path) -> None:
    ...
    q, m, w = (
        _section("questions", [], t.thesis_id),
        _section("memories", [], t.thesis_id),
...
def test_backdated_apply_full_persists_snapshot_only(tmp_path: Path) -> None:
    ...
    _thesis_rows(snap, "claims")
    trig = r.create_trigger(
        tid, claim_ids=[cid], canonical_refs=["canon:bk"], summary="backdated trigger", summary_origin="deterministic"
    )
    ...
    out = r.apply_research_result(
        tid,
        {
            ...
            "state": {
                "thesis_id": tid,
                "assessment": "supported",
                "claim_assessments": {cid: "supported"},
                "expression_assessments": {},
            },
