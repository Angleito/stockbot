"""Expression assessment (runner) + deterministic monitoring (monitor.tick)."""
...
from app.thesis.runner import ThesisResearchResult, run_trigger
...
# -- group 3: expression assessment via fake-gateway run_trigger outcomes --------

def test_timing_mismatch_flagged_without_changing_thesis_state(tmp_path):
    ...
    gw = _GW({"expression_updates": [{"expression_id": eid, "status": "flagged"}],
              "evidence_refs": [{"canonical_ref": "ev:m", "summary": "30-day puts noted",
                                 "known_at": T1}],
              "counterevidence": "near-term filing cuts against puts timing",
              "alternative": "thesis stays long-term bullish",
              "journal_summary": "timing mismatch: 30-day puts vs 2-year thesis"})
    out = run_trigger(r, t.thesis_id, trig.trigger_id, gw, _ctx(), known_at=T2)
    assert out.processed and gw.calls == 1
    assert r.load_thesis(t.thesis_id).expressions[0].status == "flagged"
    assert r.load_state(t.thesis_id).assessment == "unresolved"
    body = Path(out.journal_path).read_text(encoding="utf-8")
    assert "timing mismatch" in body and "cuts against" in body


def test_bullish_equity_asks_no_options_questions(tmp_path):
    ...
    gw = _GW({"evidence_refs": [{"canonical_ref": "ev:e", "summary": "steady demand",
                                 "known_at": T1}], "journal_summary": "equity review"})
    run_trigger(r, t.thesis_id, trig.trigger_id, gw, _ctx(), known_at=T2)
    questions = r.load_questions(t.thesis_id)
    assert questions == []
    assert r.load_state(t.thesis_id).assessment == "unresolved"


def test_covered_call_without_portfolio_leaves_ownership_unresolved(tmp_path):
    r, t = _make(tmp_path, exprs=[{"instrument": "option", "direction": "long",
                                   "structure": "covered call", "status": "active"}])
    full = r.load_thesis(t.thesis_id)
    trig = r.create_trigger(t.thesis_id, claim_ids=[full.claims[0].claim_id],
                            canonical_refs=["ev:c"], summary="s")
    gw = _GW({"evidence_refs": [{"canonical_ref": "ev:c", "summary": "call overlay noted",
                                 "known_at": T1}],
              "journal_summary": "ownership unresolved without portfolio read"})
    out = run_trigger(r, t.thesis_id, trig.trigger_id, gw, _ctx(), known_at=T2)
    assert "get_portfolio_snapshot" not in out.tools_used
    assert r.load_thesis(t.thesis_id).expressions[0].deterministic_support == "unknown"
    assert "unresolved" in Path(out.journal_path).read_text(encoding="utf-8")


def test_long_puts_without_market_has_no_invented_prices_or_greeks(tmp_path):
    r, t = _make(tmp_path, exprs=[{"instrument": "option", "direction": "long",
                                   "structure": "long puts", "status": "active"}])
    full = r.load_thesis(t.thesis_id)
    trig = r.create_trigger(t.thesis_id, claim_ids=[full.claims[0].claim_id],
                            canonical_refs=["ev:p"], summary="s")
    gw = _GW({"evidence_refs": [{"canonical_ref": "ev:p", "summary": "puts noted, no quote available",
                                 "known_at": T1}],
              "journal_summary": "not evaluable without market read; no quotes fetched"})
    out = run_trigger(r, t.thesis_id, trig.trigger_id, gw, _ctx(), known_at=T2)
    assert "get_market_snapshot" not in out.tools_used
    body = Path(out.journal_path).read_text(encoding="utf-8")
...
def test_irrelevant_event_produces_zero_pi_calls(tmp_path):
    r, t = _make(tmp_path)
    gw = _GW()
...
def test_duplicate_event_produces_zero_second_pi_call(tmp_path):
    r, t = _make(tmp_path)
...
def test_identical_events_within_one_tick_coalesce_to_one_call(tmp_path):
    r, t = _make(tmp_path)
...
def test_relevant_filing_produces_one_bounded_call(tmp_path):
    r = ThesisRepository(tmp_path / "theses")
    ...
    gw = _GW({"evidence_refs": [{"canonical_ref": "edgar:NVDA:10-K:f1",
                                 "summary": "10-K notes steady demand", "known_at": T1}],
              "counterevidence": "risk factors mention cyclicality",
              "journal_summary": "reviewed filing"})
    res = tick(r, t.thesis_id, {"sec_filings": FilingSrc()}, gw, _ctx(), known_at=T2)
    assert gw.calls == 1 and len(res.triggers_created) == 1
    assert "cyclicality" in Path(res.runs[0].journal_path).read_text(encoding="utf-8")


def test_simultaneous_meaningful_events_produce_bounded_calls(tmp_path):
    r, t = _make(tmp_path)
    gw = _GW({"journal_summary": "ok"})
...
def test_possible_invalidator_produces_one_call(tmp_path):
    ...
    r.apply_research_result(t.thesis_id, {"evidence_refs": [
        ...
        "run:seed")
    gw = _GW({"journal_summary": "invalidator check"})
    res = tick(r, t.thesis_id, {}, gw, _ctx(), known_at=T2)
    assert gw.calls == 1 and len(res.triggers_created) == 1


def test_expression_impact_event_tags_expression_ids(tmp_path):
...
def test_source_failure_leaves_checkpoint_unadvanced(tmp_path):
    r, t = _make(tmp_path)
...
def test_restart_processes_pending_once_without_dupes(tmp_path):
    r, t = _make(tmp_path)
    src = _Src([_ev("ev:1")])
    bad = _GW(fail=True)
...
def test_paused_and_closed_query_no_sources(tmp_path):
    r, t = _make(tmp_path)
    gw = _GW()
...
def test_two_theses_stay_isolated(tmp_path):
    r = ThesisRepository(tmp_path / "theses")
...
def test_evidence_refs_stay_compact_and_reject_bodies(tmp_path):
    r, t = _make(tmp_path)
    full = r.load_thesis(t.thesis_id)
    trig = r.create_trigger(t.thesis_id, claim_ids=[full.claims[0].claim_id],
                            canonical_refs=["ev:b"], summary="s")
    gw = _GW({"evidence_refs": [{"canonical_ref": "ev:b", "summary": "note",
    ...
    with pytest.raises(ValueError):
        ThesisResearchResult.from_dict(
            {"evidence_refs": [{"canonical_ref": "x", "summary": "s",
                                "known_at": T1, "content": "smuggled"}]},
            thesis_id=t.thesis_id, claim_ids=set(), expression_ids=set(),
            question_ids=set(), known_at=T2)
