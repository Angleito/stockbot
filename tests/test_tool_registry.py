...
THESIS_WRITE_TOOLS = frozenset({"thesis_create", "thesis_refine", "thesis_watch", "thesis_journal"})


def test_thesis_domains_and_structured_schemas():
    assert TOOL_DOMAINS["thesis_show"] == "thesis_read"
    assert {TOOL_DOMAINS[name] for name in THESIS_WRITE_TOOLS} == {"thesis_write"}
