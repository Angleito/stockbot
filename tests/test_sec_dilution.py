...
def test_missing_or_zero_existing_not_quantifiable() -> None:
    assert dilution.dilution_profile(existing_shares=None, new_shares=25)["dilution_pct"] == "not_quantifiable"
    assert dilution.dilution_profile(existing_shares=0, new_shares=25)["dilution_pct"] == "not_quantifiable"
    assert dilution.dilution_profile(existing_shares=100, new_shares=None)["dilution_pct"] == "not_quantifiable"
...
def test_inputs_formulas_accessions_echoed() -> None:
    out = dilution.dilution_profile(
        existing_shares=100, new_shares=25, market_cap=1000, atm_size=100, source_accessions=["a1"]
    )
    inputs = _as_dict(out["inputs"])
    assert inputs["existing_shares"] == 100
    assert inputs["new_shares"] == 25
    formulas = _as_dict(out["formulas"])
    assert formulas["dilution_pct"] == "dilution_pct = new_shares / (existing_shares + new_shares) * 100"
    assert formulas["atm_pct_of_market_cap"] == "atm_pct_of_market_cap = atm_size / market_cap * 100"
    assert formulas["fully_diluted_shares"] == "fully_diluted_shares = existing + new + convertible + warrant"
...
def test_profile_observes_without_computing(monkeypatch: pytest.MonkeyPatch) -> None:
    history = [
        SimpleNamespace(form="424B5", shares=100, accession_no="a1"),
    ...
    assert (
        "424B share counts are summed across disclosures without deduplication. This is not confirmed issuance and must not be interpreted as incremental dilution."
        in note
    )
