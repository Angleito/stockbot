...
def test_validate_claims_rejects_bad_shapes():
    assert validate_claims(None) is None
    assert validate_claims([]) is None
    assert validate_claims({}) is None
    assert validate_claims({"claims": "nope"}) is None
    assert validate_claims({"claims": [{"claim": 1}]}) is None
    assert validate_claims({"claims": [{"claim": "x", "source_url": 2}]}) is None
    assert validate_claims({"claims": [{"claim": "x", "source_url": "u", "published_at": 1}]}) is None
    assert validate_claims({"claims": [{"claim": "x", "source_url": "u", "quote_or_evidence": None}]}) is None
    ...
    assert validate_claims({"claims": [long_claim]}) is None
    ...
    monkeypatch.setattr(
        ...
        lambda model, prompt, max_tokens=2000: json.dumps({
            "claims": [{
                ...
                "source_url": "https://example.com/a",
                "published_at": None,
                "quote_or_evidence": "AMD grew revenue.",
            }]
        }),
    )
...
def test_reader_prompt_instructs_to_ignore_instructions():
    ...
    assert '"claim"' in READER_PROMPT_TEMPLATE
