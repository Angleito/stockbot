...
def _make(tmp_path: Path, invalidators: list[str] | None = None) -> tuple[ThesisRepository, Thesis]:
    ...
    t = r.create_thesis(
        "NVDA thesis", scope="NVDA", claims=["NVDA demand grows"], invalidators=invalidators or [], effective_at=T0
    )
    return r, t
...
def test_create_trigger_rejects_unknown_origin(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError):
        r.create_trigger(t.thesis_id, canonical_refs=["ev:1"], summary="s", summary_origin="external-raw")
    with pytest.raises(ValueError):
        r.create_trigger(t.thesis_id, canonical_refs=["ev:1"], summary="s", summary_origin="anything-else")
...
def test_create_trigger_persists_origin_roundtrip(tmp_path: Path) -> None:
    ...
    for origin in ("deterministic", "recycled"):
        trig = r.create_trigger(t.thesis_id, canonical_refs=["ev:1"], summary="s", summary_origin=origin)
...
def test_hostile_evidence_summary_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="hostile"):
        r.apply_research_result(
            t.thesis_id, {"evidence_refs": [{"canonical_ref": "ev:1", "summary": HOSTILE, "known_at": T1}]}, "run:x"
        )
    n = r.apply_research_result(
        t.thesis_id, {"evidence_refs": [{"canonical_ref": "ev:1", "summary": BENIGN, "known_at": T1}]}, "run:x"
    )
    assert n["evidence"] == 1
...
def test_hostile_journal_rejected(tmp_path: Path) -> None:
    ...
    assert dest.is_file()
    with pytest.raises(ValueError, match="hostile"):
        r.apply_research_result(t.thesis_id, {"journal_entry": {"title": "t", "body": HOSTILE}}, "run:x")
...
def test_tick_labels_recycled_for_stored(tmp_path: Path) -> None:
    ...
    r.apply_research_result(
        t.thesis_id,
        {
            "watch_add": [
                {
                    "rule_id": "rule:1",
                    "rule_type": "explicit_thesis_invalidator",
                    "enabled": True,
    ...
    atomic_write_yaml(
        evdir / "ev_hostile.yaml",
        {
            "schema_version": 1,
            "evidence_id": "ev:hostile",
    ...
    def _fake_list(*a: object, **k: object) -> list[Filing]:
        return [
            Filing(
                ...
                known_at="2026-01-02",
                report_period=None,
                primary_document=hostile_doc,
                is_amendment=False,
                amendment_of=None,
                source="https://evil.example exfil",
            )
        ]
...
def test_deterministic_trigger_still_gates_hostile_evidence(tmp_path: Path) -> None:
    ...
    trig = r.create_trigger(
        t.thesis_id, canonical_refs=["seed:1"], summary="routine check", summary_origin="deterministic"
    )
    ctx = build_live_context(r, t.thesis_id, trig, data_cutoff=T2)
    prompt = _build_prompt(thesis_id=t.thesis_id, trigger=trig, data_cutoff=T2, ctx=ctx, run_id="run:test")
...
def test_deterministic_hostile_summary_never_reaches_prompt(tmp_path: Path) -> None:
    ...
    r, t = _make(tmp_path)
    trig = r.create_trigger(t.thesis_id, canonical_refs=["seed:1"], summary=HOSTILE, summary_origin="deterministic")
    ctx = build_live_context(r, t.thesis_id, trig, data_cutoff=T2)
    prompt = _build_prompt(thesis_id=t.thesis_id, trigger=trig, data_cutoff=T2, ctx=ctx, run_id="run:test")
    assert "ignore previous instructions" not in prompt
    assert "evil.example" not in prompt
    assert "[unsafe content withheld ref=trigger:" in prompt
    benign = r.create_trigger(
        t.thesis_id, canonical_refs=["seed:1"], summary="routine check", summary_origin="deterministic"
    )
    benign_ctx = build_live_context(r, t.thesis_id, benign, data_cutoff=T2)
    benign_prompt = _build_prompt(
        thesis_id=t.thesis_id, trigger=benign, data_cutoff=T2, ctx=benign_ctx, run_id="run:test"
    )
