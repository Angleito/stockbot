...
def test_current_date_unusable_price_yields_null(
    store: Path, monkeypatch: pytest.MonkeyPatch, price: float | None
) -> None:
    _seed_ko(store)
    monkeypatch.setattr(sec_facts, "_today", lambda: _dt.date(2026, 8, 10))
...
def test_dividend_recency_boundary_days(
    store: Path, monkeypatch: pytest.MonkeyPatch, age_days: int, expected_status: str, expected_ttm: float | None
) -> None:
    _fail_on_price(monkeypatch)
...
def test_monthly_payer_ttm(store: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _seed_dividends(
        store, KO_CIK, [_div_fact(0.20, s, e, 2026, "M", "2026-08-01", f"m{i}") for i, (s, e) in enumerate(months)]
    )
    result = sec_facts.get_fundamentals("KO", "dividends", as_of=AS_OF)
    assert result["ttm_dividend_per_share"] is None
    assert result["dividend_status"] == "unknown"
