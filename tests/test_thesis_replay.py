        ...
        if n == 1:  # T3 relevant filing
            self.repository.apply_research_result(
                thesis_id,
                {
                    ...
                    "evidence_refs": [
                        {
                            "canonical_ref": "edgar:NVDA:10-K:t3",
            ...
            self.repository.apply_research_result(
                thesis_id,
                {
                    ...
                    "evidence_refs": [
                        {
                            "canonical_ref": "ext:nvda-note:t4",
                    ...
                    "journal_entry": {
                        "entry_id": "journal:t4",
                        "title": "t",
                        "body": "T4 counterevidence recorded: pushout note contradicts steady-demand claim",
                        "known_at": T4,
                        "run_id": rid,
                    },
                },
            ...
            self.repository.apply_research_result(
                thesis_id,
                {
                    ...
                    "evidence_refs": [
                        {
                            "canonical_ref": "edgar:NVDA:8-K:t5",
                    ...
                    "journal_entry": {
                        "entry_id": "journal:t5",
                        "title": "t",
                        "body": "T5 major change reviewed: guidance cut challenges accumulation timing",
                        "known_at": T5,
                        "run_id": rid,
                    },
                },
...
def test_replay_t0_through_t5_point_in_time_and_idempotent(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    src = _ReplaySource(
        [
            CanonicalEvent(
                event_id="t1",
                canonical_ref="ext:other:t1",
                source="sec_filings",
                known_at=T1,
                entity="UNRELATEDCORPXYZ",
                summary="unrelated files",
            ),
            CanonicalEvent(
                event_id="t2",
                canonical_ref="ext:other:t1",
            ...
            CanonicalEvent(
                ...
                known_at=T3,
                entity="NVDA",
                summary="NVDA 10-K notes steady demand",
            ),
            CanonicalEvent(
                event_id="t4",
                canonical_ref="ext:nvda-note:t4",
            ...
            CanonicalEvent(
                ...
                known_at=T5,
                entity="NVDA",
                summary="NVDA 8-K major guidance cut",
            ),
        ]
    )
    ...
    tick(r, tid, {"sec_filings": src}, known_at=T3)
...
def test_journal_known_at_gates_context(tmp_path: Path) -> None:
    ...
    r.append_journal_entry(
        t.thesis_id, {"entry_id": "past", "title": "past", "body": "b", "trigger_id": trig.trigger_id, "known_at": T3}
    )
    r.append_journal_entry(
        t.thesis_id,
        {"entry_id": "future", "title": "future", "body": "b", "trigger_id": trig.trigger_id, "known_at": T5},
    )
    r.append_journal_entry(
        t.thesis_id, {"entry_id": "noka", "title": "noka", "body": "b", "trigger_id": trig.trigger_id}
    )
...
def test_external_evidence_rule_loads_unsupported_and_never_live(
    ...
    rules.append(
        {
            "rule_id": "rule:ext",
            "rule_type": "new_external_evidence",
            "enabled": True,
            "support_status": "supported",
            "support_reason": "",
            "claim_ids": [cid],
