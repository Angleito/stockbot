...
def test_replay_t0_through_t5_point_in_time_and_idempotent(tmp_path):
    ...
    tid = t.thesis_id
    src = _ReplaySource([
        CanonicalEvent(event_id="t1", canonical_ref="ext:other:t1", source="external_evidence",
                       known_at=T1, entity="UNRELATEDCORPXYZ", summary="unrelated files"),
        CanonicalEvent(event_id="t2", canonical_ref="ext:other:t1", source="external_evidence",
                       known_at=T2, entity="UNRELATEDCORPXYZ", summary="unrelated files"),
        CanonicalEvent(event_id="t3", canonical_ref="edgar:NVDA:10-K:t3", source="external_evidence",
                       known_at=T3, entity="NVDA", summary="NVDA 10-K notes steady demand"),
        CanonicalEvent(event_id="t4", canonical_ref="ext:nvda-note:t4", source="external_evidence",
                       known_at=T4, entity="NVDA", summary="note warns of NVDA order pushouts"),
        CanonicalEvent(event_id="t5", canonical_ref="edgar:NVDA:8-K:t5", source="external_evidence",
                       known_at=T5, entity="NVDA", summary="NVDA 8-K major guidance cut"),
    ])
    gw = _ReplayGateway()
    ctx = _ctx()

    res1 = tick(r, tid, {"external_evidence": src}, gw, ctx, known_at=T1)
    assert gw.calls == 0 and res1.triggers_created == [] and _journals(root, t.slug) == []
    res2 = tick(r, tid, {"external_evidence": src}, gw, ctx, known_at=T2)
    assert gw.calls == 0 and res2.triggers_created == [] and _journals(root, t.slug) == []

    res3 = tick(r, tid, {"external_evidence": src}, gw, ctx, known_at=T3)
    ...
    res4 = tick(r, tid, {"external_evidence": src}, gw, ctx, known_at=T4)
    ...
    res5 = tick(r, tid, {"external_evidence": src}, gw, ctx, known_at=T5)
    ...
    again = tick(r, tid, {"external_evidence": src}, gw, ctx, known_at=T5)
