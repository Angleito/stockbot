...
def test_xbrl_lineage_as_of_excludes_restatement(tmp_path: Path) -> None:
    ...
    assert parquet.write_rows("financial_facts", rows, root=tmp_path / "parquet") == 2
    early = lineage.xbrl_lineage("E1", "Revenues", as_of="2025-02-15", root=tmp_path)
    ...
    with pytest.raises(ValueError):
        lineage.xbrl_lineage("E1", "Revenues", as_of="02/15/2025", root=tmp_path)
