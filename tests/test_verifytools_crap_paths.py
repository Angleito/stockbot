...
def _event(conn: sqlite3.Connection, seq: int, etype: str, name: str, at: str, args: str | None = None) -> None:
    conn.execute(
...
def _pass_db(path: Path, tool: str = "get_fundamentals") -> Path:
    ...
    _event(conn, 1, "tool_started", "call_tool", "2026-01-01T00:00:01+00:00", json.dumps({"name": tool}))
...
def test_expected_args_mismatch_db_and_row_errors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        assert v._expected_args_mismatch(conn2, "get_fundamentals", {"ticker": "AAPL"}) is False
    finally:
        conn2.close()
...
def test_verification_args_thesis_and_research_substitution(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    out = v._verification_args("thesis_show", {"id": "thesis-placeholder"}, tmp_path, tmp_path / "d")
    assert out == {"id": "thesis-1"}
    out2 = v._verification_args(
        "research_status", {"session_id": "research-session-placeholder"}, tmp_path, tmp_path / "d"
    )
...
def _lines(a: str = "discover-1", b: str = "discover-2") -> list[str]:
    return [
        json.dumps({"id": a, "tools": [], "bridge_ok": True}),
...
def test_run_confusion_case_fail_and_pair_tally(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _cat_select(
        result: v.AttemptResult, db_path: Path, expected: str, base_args: dict[str, object]
    ) -> v.RoutingFailureCategory | None:
    ...
    ok, total = v._run_confusion_pair(
        ("a", "b"),
        [
            {"expected_tool": "a", "prompt": "p", "arguments": {}, "pair": ["a", "b"]},
...
def test_run_holdout_no_cases_and_loop(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(v, "tool_schemas", dict)
