...
def test_refresh_data_offline_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    assert (
        raw_archive.find(
            "finra",
            "data_page",
            "otcMarket/consolidatedShortInterest:2026-08-14:offset0",
            root=tmp_path / "raw",
        )
        is not None
    )
    assert (
        raw_archive.find(
            "finra",
            "data_page",
            "otcMarket/consolidatedShortInterest:2026-08-14:offset2",
            root=tmp_path / "raw",
        )
        is not None
    )
...
def test_backfill_finra_known_at_rewrites_only_settlement_stamped(tmp_path: Path):
    ...
    parquet.write_rows(
        ...
        [
            {
                "row_id": "finra:row:2026-08-14:AAA:oldhash1234",
                "entity_id": None,
                "security_id": None,
                "symbol_code": "AAA",
                "issue_name": "Alpha",
                ...
                "content_hash": "old",
                "parser_version": "finra-short-interest-v1",
            }
        ],
...
def test_refresh_repairs_settlement_stamped_row_on_same_snapshot(
    ...
    snapshot_hash = hashlib.sha256(json.dumps(payload, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
    parquet.write_rows(
        "short_interest",
        [
            {
                "row_id": f"finra:row:{settlement}:AAA:{snapshot_hash[:12]}",
                "entity_id": None,
                "security_id": None,
                "symbol_code": "AAA",
                "issue_name": "Alpha",
...
def test_backfill_targets_only_legacy_v1_rows(tmp_path: Path) -> None:
    ...
    parquet.write_rows(
        ...
        [
            ...
            {
                "row_id": "finra:row:2026-08-14:BBB:newhash5678",
                "entity_id": None,
                "security_id": None,
                "symbol_code": "BBB",
                "issue_name": "Beta",
...
def test_short_interest_has_legacy_v1_probe(tmp_path: Path) -> None:
    ...
    v2_rows: list[dict[str, object]] = [
        {
            "row_id": f"finra:row:2026-08-14:{sym}:v2hash{i:04d}",
            "entity_id": None,
            "security_id": None,
            "symbol_code": sym,
            "issue_name": sym,
    ...
    parquet.write_rows(
        ...
        [
            {
                "row_id": "finra:row:2026-08-14:DDD:oldhash1234",
                "entity_id": None,
                "security_id": None,
                "symbol_code": "DDD",
                "issue_name": "Delta",
