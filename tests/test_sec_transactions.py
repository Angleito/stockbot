...
def test_tender_offer_announced_with_per_share():
    ...
    txn = normalize_transaction("acc-1", "SC TO-T", target="tgt", filed_at="2024-01-01", text=text)
...
def test_update_unions_accessions_and_status_wins():
    prev = normalize_transaction("acc-1", "SC TO-T", target="TGT", filed_at="2024-01-01", text="Offers $10 per share.")
    curr = normalize_transaction(
        "acc-2", "SC TO-T/A", target="TGT", filed_at="2024-02-01", text="Offers $12 per share."
    )
...
def test_get_transaction_status_newest_first(monkeypatch: pytest.MonkeyPatch) -> None:
    filings = [
        SimpleNamespace(accession_no="new", form="SC TO-T/A", filed_at="2024-02-01", company="tgt"),
        SimpleNamespace(accession_no="old", form="SC TO-T", filed_at="2024-01-01", company="tgt"),
    ]
