...
`http://127.0.0.1:8080` (host `~/.pi/agent/models.json` providers `liquid-local`, `qwen-local`, `minicpm-local`, `local-qwen`); IDs are the router preset IDs (`LiquidAI/LFM2.5-1.2B-Thinking-GGUF:Q4_K_M`, `ggml-org/Qwen3.6-35B-A3B-GGUF:Q4_K_M`, `openbmb/MiniCPM5-1B-GGUF:Q4_K_M`) — one port for all models — and need no sandbox egress.
